...
function New-DefaultTokenSettings {
    $defaultGeminiCommand = '$authPath = "$env:APPDATA\TokenMonitor\gemini_auth.json"; if (-not (Test-Path -LiteralPath $authPath)) { [PSCustomObject]@{ Error = "No gemini_auth.json" } | ConvertTo-Json -Compress; exit }; try { $auth = Get-Content -LiteralPath $authPath -Raw | ConvertFrom-Json; $psid = $auth.Secure_1PSID; $sapisid = $auth.SAPISID; if (-not $psid -or -not $sapisid) { [PSCustomObject]@{ Error = "Missing credentials" } | ConvertTo-Json -Compress; exit }; $now = [int][DateTimeOffset]::UtcNow.ToUnixTimeSeconds(); $inputStr = "$now $sapisid https://gemini.google.com"; $hasher = [System.Security.Cryptography.HashAlgorithm]::Create("SHA1"); $hashBytes = $hasher.ComputeHash([System.Text.Encoding]::UTF8.GetBytes($inputStr)); $hashHex = [System.BitConverter]::ToString($hashBytes).Replace("-", "").ToLower(); $authHeader = "SAPISIDHASH ${now}_${hashHex}"; $headers = @{ Authorization = $authHeader; "User-Agent" = "Mozilla/5.0"; "x-same-domain" = "1"; Cookie = "__Secure-1PSID=$psid; SAPISID=$sapisid" }; $body = @{ "f.req" = ''[[["jSf9Qc","[]",null,"generic"]]]'' }; $resp = Invoke-RestMethod -Uri "https://gemini.google.com/_/BardChatUi/data/batchexecute?rpcids=jSf9Qc&source-path=%2Fusage" -Headers $headers -Method Post -Body $body -ErrorAction Stop; $patternSimple = ''\[2,\[\[\d+,\s*[\d\.\-e]+,\s*1,.*?\]\]\],(?:true|false)\]''; if ($resp -match $patternSimple) { $matched = $Matches[0]; $payload = ConvertFrom-Json $matched; $items = $payload[1]; $fivePercent = $null; $weekPercent = $null; foreach ($item in $items) { $remaining = [Math]::Max(0.0, [Math]::Min(100.0, [Math]::Round((1.0 - [double]$item[1]) * 100.0, 1))); if ($item[2] -eq 1) { $fivePercent = $remaining } elseif ($item[2] -eq 2) { $weekPercent = $remaining } }; [PSCustomObject]@{ fiveHourRemainingPercent = $fivePercent; weeklyRemainingPercent = $weekPercent } | ConvertTo-Json -Compress } else { [PSCustomObject]@{ Error = "Invalid API response" } | ConvertTo-Json -Compress } } catch { [PSCustomObject]@{ Error = $_.Exception.Message } | ConvertTo-Json -Compress }'
...
function Format-TokenUsageTooltip {
    ...
    foreach ($provider in @($Snapshot.Providers)) {
        ...
        $percent = $provider.WeeklyRemainingPercent
        if ($null -ne $provider.FiveHourRemainingPercent -and $null -ne $provider.WeeklyRemainingPercent) {
            $percent = [Math]::Min([double]$provider.FiveHourRemainingPercent, [double]$provider.WeeklyRemainingPercent)
        }
        elseif ($null -eq $percent) {
            $percent = $provider.FiveHourRemainingPercent
        }

        $parts.Add(('{0}:{1}' -f $shortName, (Format-Percent -Value $percent)))
    }
