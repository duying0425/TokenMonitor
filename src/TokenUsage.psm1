...
function New-DefaultTokenSettings {
    $defaultGeminiCommand = '$authPath = "$env:APPDATA\TokenMonitor\gemini_auth.json"; if (-not (Test-Path -LiteralPath $authPath)) { [PSCustomObject]@{ Error = "No gemini_auth.json" } | ConvertTo-Json -Compress; exit }; try { $auth = Get-Content -LiteralPath $authPath -Raw | ConvertFrom-Json; $psid1 = $auth."__Secure-1PSID"; if (-not $psid1) { $psid1 = $auth.Secure_1PSID }; if (-not $psid1) { $psid1 = $auth.SECURE_1PSID }; if (-not $psid1) { [PSCustomObject]@{ Error = "Missing __Secure-1PSID in gemini_auth.json" } | ConvertTo-Json -Compress; exit }; $sapisid = $auth.SAPISID; if (-not $sapisid) { $sapisid = $auth.sapisid }; $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession; $uri = New-Object System.Uri("https://gemini.google.com"); foreach ($prop in $auth.PSObject.Properties) { if ($prop.Value -and $prop.Value -is [string]) { $name = $prop.Name; if ($name -eq "Secure_1PSID" -or $name -eq "SECURE_1PSID") { $name = "__Secure-1PSID" } elseif ($name -eq "Secure_1PSIDTS" -or $name -eq "SECURE_1PSIDTS") { $name = "__Secure-1PSIDTS" } elseif ($name -eq "Secure_1PAPISID" -or $name -eq "SECURE_1PAPISID") { $name = "__Secure-1PAPISID" } elseif ($name -eq "Secure_3PAPISID" -or $name -eq "SECURE_3PAPISID") { $name = "__Secure-3PAPISID" }; try { $session.Cookies.Add($uri, (New-Object System.Net.Cookie($name, $prop.Value))) } catch {} } }; $authHeader = $null; if ($sapisid) { $now = [int][DateTimeOffset]::UtcNow.ToUnixTimeSeconds(); $mkHash = { param($key) $bytes = [System.Security.Cryptography.HashAlgorithm]::Create("SHA1").ComputeHash([System.Text.Encoding]::UTF8.GetBytes("$now $key https://gemini.google.com")); [System.BitConverter]::ToString($bytes).Replace("-","").ToLower() }; $h = & $mkHash $sapisid; $authHeader = "SAPISIDHASH ${now}_${h}"; $h1 = $auth."__Secure-1PAPISID"; if (-not $h1) { $h1 = $auth.Secure_1PAPISID }; if ($h1) { $h1hash = & $mkHash $h1; $authHeader += " SAPISID1PHASH ${now}_${h1hash}" }; $h3 = $auth."__Secure-3PAPISID"; if (-not $h3) { $h3 = $auth.Secure_3PAPISID }; if ($h3) { $h3hash = & $mkHash $h3; $authHeader += " SAPISID3PHASH ${now}_${h3hash}" } }; $hdrs = @{ "User-Agent" = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36"; "x-same-domain" = "1"; Origin = "https://gemini.google.com"; Referer = "https://gemini.google.com/usage" }; if ($authHeader) { $hdrs.Add("Authorization", $authHeader) }; $html = Invoke-RestMethod -Uri "https://gemini.google.com/usage" -Headers $hdrs -WebSession $session -Method Get -ErrorAction Stop; $at = $null; $bl = $null; $fsid = $null; if ($html -match ''"SNlM0e"\s*:\s*"([^"]+)"'') { $at = $Matches[1] }; if ($html -match ''"cfb2h"\s*:\s*"([^"]+)"'') { $bl = $Matches[1] }; if ($html -match ''"FdrFJe"\s*:\s*"([^"]+)"'') { $fsid = $Matches[1] } elseif ($html -match ''"FdrFJe"\s*:\s*(-?\d+)'') { $fsid = $Matches[1] }; $idx = $html.IndexOf("WIZ_global_data ="); if ($idx -ge 0) { $s = $idx + "WIZ_global_data =".Length; $e = $html.IndexOf("};", $s); if ($e -ge 0) { try { $d = ConvertFrom-Json $html.Substring($s, $e - $s + 1); if (-not $bl) { $bl = $d.cfb2h }; if (-not $fsid) { $fsid = $d.FdrFJe }; if (-not $at) { $at = $d.SNlM0e; if (-not $at) { $at = $d.thykhd } } } catch {} } }; if (-not $at -and ($html -match ''"thykhd"\s*:\s*"([^"]+)"'')) { $at = $Matches[1] }; if (-not $at -and ($html -match ''"((?:AD1_|AFWLb)[A-Za-z0-9\-_:%]+)"'')) { $at = $Matches[1] }; if (-not $at) { [PSCustomObject]@{ Error = "Could not extract CSRF token from Gemini usage page" } | ConvertTo-Json -Compress; exit }; $reqStr = ''[[["jSf9Qc","[]",null,"generic"]]]''; $bodyStr = "f.req=" + [Uri]::EscapeDataString($reqStr) + "&at=" + [Uri]::EscapeDataString($at); $postUrl = "https://gemini.google.com/_/BardChatUi/data/batchexecute?rpcids=jSf9Qc&source-path=%2Fusage"; if ($bl) { $postUrl += "&bl=" + [Uri]::EscapeDataString($bl) }; if ($fsid) { $postUrl += "&f.sid=" + [Uri]::EscapeDataString($fsid) }; $postUrl += "&hl=en&_reqid=$(Get-Random -Min 1000000 -Max 9999999)&rt=c"; $resp = Invoke-RestMethod -Uri $postUrl -Headers $hdrs -WebSession $session -Method Post -Body $bodyStr -ContentType "application/x-www-form-urlencoded;charset=UTF-8" -ErrorAction Stop; if ($resp -match ''"jSf9Qc","(\[.*?\])"'') { $payload = ConvertFrom-Json $Matches[1]; $items = $payload[1]; $fivePercent = $null; $weekPercent = $null; foreach ($item in $items) { $rem = [Math]::Max(0.0, [Math]::Min(100.0, [Math]::Round((1.0 - [double]$item[1]) * 100.0, 1))); if ($item[2] -eq 1) { $fivePercent = $rem } elseif ($item[2] -eq 2) { $weekPercent = $rem } }; [PSCustomObject]@{ fiveHourRemainingPercent = $fivePercent; weeklyRemainingPercent = $weekPercent } | ConvertTo-Json -Compress } else { [PSCustomObject]@{ Error = "Unexpected API response" } | ConvertTo-Json -Compress } } catch { [PSCustomObject]@{ Error = $_.Exception.Message } | ConvertTo-Json -Compress }'
...
function Read-TokenMonitorSettings {
    ...
    foreach ($provider in $settings.Providers) {
        ...
        if (-not (Get-Member -InputObject $provider -Name Command -MemberType NoteProperty -ErrorAction SilentlyContinue)) {
            ...
            if ($provider.Id -eq 'antigravity' -and ([string]::IsNullOrWhiteSpace($provider.Command) -or ($provider.Command).Contains('if (-not $sapisid -or -not $psid1)') -or ($provider.Command).Contains('$at = $d.thykhd;'))) {
